...
const PrivacyPolicyFront = () => {
    ...
    useEffect(() => {
        const fetchPolicy = async () => {
            try {
                const res = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/settings/privacy_policy`);
                if (res.ok) {
        ...
        <div className="dy-landing">
            ...
            <section style={{ padding: '5rem 0 7rem', background: '#fff' }}>
                <div className="dy-container" style={{ maxWidth: '900px' }}>
                    ...
                    {loading ? (
                        <div style={{ textAlign: 'center', padding: '3rem 0' }}>Loading privacy policy...</div>
                    ) : content ? (
                        <div
                        ...
                        <div style={{ textAlign: 'center', padding: '3rem 0' }}>
                            No privacy policy found. Please check back later.
                        </div>
                    )}
                </div>
            </section>
